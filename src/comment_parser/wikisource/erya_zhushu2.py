...
class Paragraph:
	...
	def parse(text, last_preceeding_original_paragraph):
		...
		elif text[0] == "疏" or text[0:2] == "○注" or text[0:3] == "【疏】" or text[0:3] == "【注】":
	...
	_shu_zhu_explains_prefix_pattern = re.compile("「([^」]+)」。?至?【?注】?「([^」]+)」。?")
	_shu_zhu_zhi_explains_prefix_pattern = re.compile("「([^」]+)」。【注】「([^」]+)」至「([^」]+)」。")
	_shu_zhi_zhu_explains_prefix_pattern = re.compile("「([^」]+)」至「([^」]+)」。【注】「([^」]+)」。")
	_prefix_end_delimiter_pattern = re.compile(r"(○?\s?(?=(釋曰)))|○|(釋在上。)|(者，)")
	...
	def get_regions_from_explains_prefix(self, prefix, typus):
		...
		shu_zhu_match = self._shu_zhu_explains_prefix_pattern.fullmatch(prefix)
		if shu_zhu_match != None:
			return self._get_regions_from_full_explains_prefix(shu_zhu_match.group(1), typus) + self._get_regions_from_full_explains_prefix(shu_zhu_match.group(2), "comment")
		
		shu_zhu_zhi_match = self._shu_zhu_zhi_explains_prefix_pattern.fullmatch(prefix)
		if shu_zhu_zhi_match != None:
			return self._get_regions_from_full_explains_prefix(shu_zhu_zhi_match.group(1), typus) + self._get_regions_from_a_zhi_b_explains_prefix(shu_zhu_zhi_match.group(2), shu_zhu_zhi_match.group(3), "comment")
		
		shu_zhi_zhu_match = self._shu_zhi_zhu_explains_prefix_pattern.fullmatch(prefix)
		if shu_zhi_zhu_match != None:
			return self._get_regions_from_a_zhi_b_explains_prefix(shu_zhi_zhu_match.group(1), shu_zhi_zhu_match.group(2), typus) + self._get_regions_from_full_explains_prefix(shu_zhi_zhu_match.group(3), "comment")
...
class Text:
	...
	def _normalize(raw):
		...
		rst = re.sub("○?注「(?P<a>(《|》|。|，|；|：|\w)+)」(?P<c>。|者)",
			r"\n【注】「\g<a>」\g<c>", rst)
		rst = re.sub(r"疏「(?P<a>(《|》|\w|，|：|。|；)+)」。?至?\n【注】「(?P<b>(《|》|\w|，|：|。|；)+)」。",
			r"【疏】「\g<a>」。【注】「\g<b>」。", rst)
		rst = re.sub(r"疏「(?P<a>(《|》|\w|，|：|。|；)+)」。\n【注】「(?P<b>(《|》|\w|，|：|。|；)+)」至「(?P<c>(《|》|\w|，|：|。|；)+)」。",
			r"【疏】「\g<a>」。【注】「\g<b>」至「\g<c>」。", rst)
		rst = re.sub(r"疏「(?P<a>(《|》|\w|，|：|。|；)+)」至「(?P<b>(《|》|\w|，|：|。|；)+)」。\n【注】「(?P<c>(《|》|\w|，|：|。|；)+)」。",
			r"【疏】「\g<a>」至「\g<b>」。【注】「\g<c>」。", rst)
		return rst
...
texts = [t for (i, r) in enumerate(raw_texts) for t in Text.parse_raw(r, i + 6)]
